...
const DecorBox = ({ setCurrState, setproductId }) => {
  ...
  const FetchData = (user, con) => {
    axios.get(`${PROXY}/decor/getall/${user?.data?._id}`, con).then((res) => {
      if (res.data.success) {
  ...
  const productDelete = (id) => {
    setDeleting(true);
    axios
      .delete(`${PROXY}/decor/delete/${id}`, config)
      .then(() => {
        FetchData(globleuser, config);
        setDeleting(false);
      })
            ...
            oneDatadone === null ? (
              <div className="box-shadow bg-white venues-card py-3 px-4">
                <div className="venue-card-header d-flex align-items-center justify-content-between mb-5">
                  <h5 className="primary-text fw-bold">Decore Listing</h5>
                  {/* <Link
                    href={{
                      pathname: user
